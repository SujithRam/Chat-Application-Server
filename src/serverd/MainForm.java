...
public class MainForm extends javax.swing.JFrame {
    ...
    Map<String, Map<String, Integer>> clientfl = new HashMap<String, Map<String, Integer>>(); //Client List with files
    ...
    public Vector clientFileDir = new Vector();
    /**
     * Server *
     */
    ServerSocket server;
    ...
    public void setSocketList(Socket socket) {
        try {
            ...
            appendMessage("[setSocketList]:Client Socket Added");
        } catch (Exception e) {
            appendMessage("[setSocketList]: " + e.getMessage());
        }
    }

    public void setClientFileList(String client, Map<String, Integer> clts) {
        try {
            clientfl.put(client, clts);
            appendMessage("[setClientList]:Client Files Added");
        } catch (Exception e) {
    ...
    public void setClientList(String client) {
        try {
            ...
            appendMessage("[setClientList]:Client Name Added");
        } catch (Exception e) {
            appendMessage("[setClientList]: " + e.getMessage());
        }
    }
    ...
    public void setClientFileDir(String s) {
        try {
            clientFileDir.add(s);
        } catch (Exception e) {
        }
    ...
    public String getClientFileDir(String client) {
        String dir = null;
        for (int x = 0; x < clientList.size(); x++) {
            if (clientList.get(x).equals(client)) {
                dir = (String) clientFileDir.get(x);
                break;
            }
        }
        return dir;
    ...
    public String findFile(String file) {
        ...
        Map<String, Integer> listofclients = new HashMap<String, Integer>();
        String host = "";
        int max=0, val=0;
        for (String key : clientfl.keySet()) {
            temp = clientfl.get(key);
            if (temp.containsKey(file)) {
                listofclients.put(key, (int)temp.get(file));
            }
        }
        for(String ky: listofclients.keySet())
        {
            val=(int)listofclients.get(ky);
            if(max<=val)
            {
                max=val;
                host=ky;
    ...
    public void incrementdownloadsuccesscount(String client, String filename)
    {
        Map<String, Integer> temp;
        temp=clientfl.get(client);
        int val=temp.get(filename);
        val=val+1;
        temp.put(filename, val);
        clientfl.remove(client);
        clientfl.put(client, temp);
    }
